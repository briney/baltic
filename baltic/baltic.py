...
class tree:  ## tree class
    ...
    def treeStats(self):
        ...
        if len(N_children) == 0:
            ...
            if (
                maxChildren == 2 and minChildren == 2
    ...
    def sortBranches(self, descending=True, sort_function=None, sortByHeight=True):
        ...
        if sortByHeight:  # Sort nodes by height and group nodes and leaves together
            ...
            for k in self.getInternal():
                ...
                nodes = sorted(
                    [x for x in k.children if x.branchType == "node"], key=sort_function
    ...
    def drawTree(self, order=None, width_function=None, pad_nodes=None, verbose=False):
        ...
        assert len(self.getExternal()) == len(
            order
        ...
        while len(drawn) != len(
            ...
            if len(self.Objects) > len(drawn):
                assert len(drawn) > storePlotted, (
                    ...
                    % (len(drawn), storePlotted, len(tree.Objects))
                )
            storePlotted = len(
                drawn
    ...
    def collapseBranches(
        ...
        if (
            ...
            assert [w.branchType for w in designated_nodes].count("node") == len(
                designated_nodes
        ...
        assert (
            len(nodes_to_delete) < len(newTree.getInternal()) - 1
        ...
        while (
            ...
            for k in sorted(
                ...
                if len(designated_nodes) == 0:
                    ...
                    assert [w.branchType for w in designated_nodes].count(
                        "node"
                    ) == len(
                        designated_nodes
                    ), "Non-node class detected in list of nodes designated for deletion"
...
def loadNewick(
    ...
    if absoluteTime == True:
        ...
        for k in ll.getExternal():
            ...
            if cerberus is not None:
                tipDates.append(
                    decimalDate(cerberus.group(1), fmt=date_fmt, variable=variableDate)
                )
        assert len(tipDates) > 0, (
    ...
    assert "name" in json_translation and (
        ...
        % (
            ", ".join(
                [
                    entry
                    for entry in ["name", "height", "absoluteTime", "length"]
                    if (entry in json_translation) == False
                ]
            )
        )
    )
