...
class Tree:
    ...
    def subtree(
        ...
        traverse_condition: Optional[Callable] = None,
        stem: bool = True,
    ):
        """ Generate a subtree (as a baltic tree object) from a traversal.
            ...
            Note - custom traversal functions can result in multi_type trees.
            If this is undesired call single_type() on the resulting subtree afterwards. """
        ...
        if subtree is None or len([k for k in subtree if k.is_leaf()]) == 0:
            ...
            if stem == True:  ## we want the stem
                local_tree.root.parent = copy.deepcopy(
                    k.parent
                )  ## means assigning an invisible parent to root
                local_tree.root.parent.height = (
                    0.0  ## set height to 0.0 so heights can be set
                )
                if local_tree.root.parent.parent:
                    local_tree.root.parent.parent = (
                        None  ## remove reference to the invisible parent's parent
                    )
            ...
            if (
                ...
                for nd in local_tree.get_internal():  ## iterate over nodes
                    nd.children = list(
                        filter(lambda k: k in subtree_set, nd.children)
                    )  ## only keep children seen in traversal
                local_tree.fixHangingNodes()

            if self.tip_map:  ## if original tree has a tipMap dictionary
                local_tree.tip_map = {
                    tipNum: self.tip_map[tipNum]
                    for tipNum in self.tip_map
                    if self.tip_map[tipNum]
                    in [w.name for w in local_tree.get_external()]
                }  ## copy over the relevant tip translations

            return local_tree

    def single_type(self):
        """ Removes any branches with a single child (multi_type nodes). """
        multi_type_nodes = [
            k for k in self.objects if k.is_node() and len(k.children) == 1
        ]
        while len(multi_type_nodes) > 0:
            multi_type_nodes = [
                k for k in self.objects if k.is_node() and len(k.children) == 1
            ]

            for k in sorted(multi_type_nodes, key=lambda x: -x.height):
                ...
                multi_type_nodes.remove(k)  ## remove old parent from multi_type nodes
    ...
    def set_absolute_time(self, date):
        ...
        for i in self.objects:  ## iterate over all objects
            i.absolute_time = (
                date - self.tree_height + i.height
            )  ## heights are in units of time from the root
        self.most_recent = max([k.absolute_time for k in self.objects])
    ...
    def tree_stats(self):
        ...
        print(
            "\nTree height: %.6f\nTree length: %.6f"
            % (self.tree_height, sum([x.length for x in obs]))
        )  ## report the height and length of tree

        nodes = self.get_internal()  ## get all nodes
        strictly_bifurcating = False  ## assume tree is not strictly bifurcating
        multi_type = False
        singleton = False
        ...
        if len(n_children) == 0:
            ...
            min_children, max_children = (
                min(n_children),
                max(n_children),
            )  ## get the largest number of descendant branches of any node
            if (
                max_children == 2 and min_children == 2
            ):  ## if every node has at most two children branches
                strictly_bifurcating = True  ## it's strictly bifurcating
            if min_children == 1:
                multi_type = True

        has_traits = False  ## assume tree has no annotations
        max_annotations = max(
            [len(x.traits) for x in obs]
        )  ## check the largest number of annotations any branch has
        if max_annotations > 0:  ## if it's more than 0
            has_traits = True  ## there are annotations

        if strictly_bifurcating:
            print("strictly bifurcating tree")  ## report
        if multi_type:
            print("multi_type tree")  ## report
        if singleton:
        ...
        print(
            f"\nNumbers of objects in tree: {len(obs)} ({len(nodes)} nodes and {len(self.get_external())} leaves)\n"
        )  ## report numbers of different objects in the tree

    def traverse_tree(
        self,
        cur_node: Optional[node] = None,
        include_condition: Optional[Callable] = None,
        traverse_condition: Optional[Callable] = None,
        collect: Optional[Iterable] = None,
        verbose: bool = False,
    ) -> Iterable:
        if cur_node == None:  ## if no starting point defined - start from root
            ...
            if (
                traverse_condition == None and include_condition == None
        ...
        if (
            ...
            for child in filter(
                ...
                if verbose:
                    print(f"visiting child {child.index}")
                self.traverse_tree(
                    cur_node=child,
                ...
                if verbose:
                    print(f"child {child.index} done")
            assert (
                len(cur_node.children) > 0
            ), f"Tried traversing through hanging node without children. Index: {cur_node.index}"
            cur_node.child_height = max(
                [
                    child.child_height if child.is_node() else child.height
            ...
            self.tree_height = (
                cur_node.child_height
            )  ## it's the highest child of the starting node
        return collect

    def rename_tips(self, d: Optional[dict] = None):
        """ Give each tip its correct label using a dictionary. """
        if d == None and self.tip_map != None:
            d = self.tip_map
    ...
    def sort_branches(
        ...
        if sort_function == None:
            sort_function = (
                lambda k: (k.is_node(), -len(k.leaves) * mod, k.length * mod)
                if k.is_node()
                else (k.is_node(), k.length * mod)
            )
        if sort_by_height:  # Sort nodes by height and group nodes and leaves together
            ...
            for k in self.get_internal():
                leaves_idx = [
                    (i, ctr)
                    for ctr, i in enumerate(k.children)
                    if i.branch_type == "leaf"
                ]  # Get original indices of leaves
                nodes = sorted(
                    [x for x in k.children if x.branch_type == "node"],
                    key=sort_function,
                )  # Sort nodes only by number of descendants
                children = nodes
                for i in leaves_idx:  # Insert leaves back into same positions
                    children.insert(i[1], i[0])
                k.children = children
        self.draw_tree()  ## update x and y positions of each branch, since y positions will have changed because of sorting

    def draw_tree(
        self,
        order: Optional[Iterable[node]] = None,
        width_function: Optional[Callable] = None,
        pad_nodes: Optional[Iterable[node]] = None,
        verbose: bool = False,
    ):
        """ 
        ...
        if order == None:
            ...
            # to make sure they're plotted in the correct order along
            # the vertical tree dimension
        ...
        while len(drawn) != len(
            ...
            if verbose:
                print(f"Drawing iteration {len(drawn)}")
            for k in filter(
                lambda w: w.index not in drawn, self.get_internal()
            ):  ## iterate through internal nodes that have not been drawn
                if len([q.y for q in k.children if q.y != None]) == len(
                    k.children
                ):  ## all y coordinates of children known
                    if verbose:
                        print(f"Setting node {k.index} coordinates to"),
                    x = k.height  ## x position is height
                    children_y_coords = [
                        q.y for q in k.children if q.y != None
                    ...
                    drawn[k.index] = None  ## remember that this objects has been drawn
                    if verbose:
                        print(f"{k.y} ({len(drawn)} branches drawn)")
                    min_yrange = min(
                        [
            ...
            if len(self.objects) > len(drawn):
                assert len(drawn) > store_plotted, (
                    ...
                    % (len(drawn), store_plotted, len(self.objects))
                )
            store_plotted = len(
                drawn
            )  ## remember how many branches were drawn this iteration

        y_vals = [k.y for k in self.objects]  ## all y values
        self.y_span = (
            max(y_vals) - min(y_vals) + min(y_vals) * 2
        )  ## determine appropriate y axis span of tree

        if self.root.is_node():
            self.root.x = min(
                [q.x - q.length for q in self.root.children if q.x != None]
    ...
    def draw_unrooted(
        ...
        if n.is_node():
            for ch in n.children:
                ...
                ch.traits["tau"] = eta
                eta += w
                self.draw_unrooted(rotate, ch, total)

    def commonAncestor(self, descendants: Iterable[node], strict: bool = False):
        """
        ...
        paths_to_root = {k.index: set() for k in descendants}
        for k in descendants:  ## iterate through every descendant
            ...
            while cur_node:  ## while not at root
                ...
                paths_to_root[k.index].add(cur_node)
    ...
    def collapse_subtree(
        ...
        given_name: str,
        verbose: bool = False,
        widthFunction: Optional[Callable] = lambda k: len(k.leaves),
    ) -> node:
        """ Collapse an entire subtree into a clade object. """
        assert cl.is_node(), "Cannot collapse non-node class"
        collapsed_clade = clade(given_name)
        collapsed_clade.index = cl.index
        collapsed_clade.leaves = cl.leaves
        collapsed_clade.length = cl.length
        collapsed_clade.height = cl.height
        collapsed_clade.parent = cl.parent
        collapsed_clade.absolute_time = cl.absolute_time
        collapsed_clade.traits = cl.traits
        collapsed_clade.width = widthFunction(cl)

        if verbose:
            print(
                f"Replacing node {cl.index} (parent {cl.parent.index}) with a clade class"
            )
        parent = cl.parent

        remove_from_tree = self.traverse_tree(cl, include_condition=lambda k: True)
        collapsed_clade.subtree = remove_from_tree
        assert len(remove_from_tree) < len(
            self.objects
        ), "Attempted collapse of entire tree"
        collapsed_clade.last_height = max([x.height for x in remove_from_tree])
        if [x.absolute_time for x in remove_from_tree].count(None) != len(
            remove_from_tree
        ):
            collapsed_clade.lastabsolute_time = max(
                [x.absolute_time for x in remove_from_tree]
            )
        ...
        parent.children.append(collapsed_clade)
        self.objects.append(collapsed_clade)
        collapsed_clade.parent = parent
        if self.tip_map != None:
            self.tip_map[given_name] = given_name

        self.traverse_tree()
        self.sort_branches()
        return collapsed_clade

    def uncollapse_subtree(self):
    ...
    def collapseBranches(
        ...
        if (
            ...
            nodes_to_delete = list(
                filter(
                    lambda n: n.is_node()
                    and collapseIf(n) == True
                    and n != newTree.root,
                    newTree.objects,
                )
            )  ## fetch a list of all nodes who are not the root and who satisfy the condition
        else:
            assert [w.branch_type for w in designated_nodes].count("node") == len(
                designated_nodes
        ...
        assert (
            len(nodes_to_delete) < len(newTree.get_internal()) - 1
        ...
        while (
            ...
            for k in sorted(
                ...
                if len(designated_nodes) == 0:
                    ...
                    assert [w.branch_type for w in designated_nodes].count(
                        "node"
                    ) == len(
                        designated_nodes
                    ), "Non-node class detected in list of nodes designated for deletion"
            ...
            for tr in traits:  ## iterate through keys
                if tr in cur_node.traits:  ## if key is available
                    ...
                    print(
                        ...
                        % (tr, cur_node.index, cur_node.branch_type)
                    )
    ...
    def allTMRCAs(self):
        tip_names = [k.name for k in self.get_external()]
        tmrcaMatrix = {
        ...
        for k in self.get_internal():  ## iterate over nodes
            ...
            for a, tipA in enumerate(all_children):
                for tipB in all_children[a + 1 :]:
                    if (
                        tmrcaMatrix[tipA][tipB] == None
                        or tmrcaMatrix[tipA][tipB] <= k.absolute_time
                    ):  ## if node's time is more recent than previous entry - set new TMRCA value for pair of tips
                        tmrcaMatrix[tipA][tipB] = k.absolute_time
                        tmrcaMatrix[tipB][tipA] = k.absolute_time
        return tmrcaMatrix
    ...
    def reduceTree(self, keep, verbose=False):
        ...
        assert len([k for k in keep if not k.is_leaf()]) == 0, (
            "Embedding contains %d non-leaf branches."
            % (len([k for k in keep if k.is_leaf() == False]))
        )
        if verbose:
        ...
        for k in reduced_tree.get_internal():  ## iterate through reduced tree
            k.children = [
        ...
        reduced_tree.sort_branches()  ## sort

        return reduced_tree  ## return new tree

    def countLineages(self, t, attr="absolute_time", condition=lambda x: True):
        return len(
            [
                k
                for k in self.objects
                if getattr(k.parent, attr) != None
                and getattr(k.parent, attr) < t <= getattr(k, attr)
                and condition(k)
            ]
        )

    def get_external(self, secondFilter=None):
        """
        Get all branches whose branch_type is "leaf".
        A function can be provided to filter internal nodes according to an additional property.
        """
        externals = list(
            filter(secondFilter, filter(lambda k: k.is_leaf(), self.objects))
        )
        return externals

    def get_internal(self, secondFilter=None):
        """
        Get all branches whose branch_type is "node".
        A function can be provided to filter internal nodes according to an additional property.
        """
        internals = list(
    ...
    def fixHangingNodes(self):
        ...
        hangingNodes = list(
            filter(hangingCondition, self.objects)
        )  ## check for nodes without any children (hanging nodes)
        while len(hangingNodes) > 0:
            for h in sorted(hangingNodes, key=lambda x: -x.height):
                h.parent.children.remove(
                    h
                )  ## remove old parent from grandparent's children
                hangingNodes.remove(h)  ## remove old parent from multi_type nodes
    ...
    def plotTree(
        ...
        **kwargs,
        ...
        line_segments = LineCollection(
            branches, lw=linewidths, color=colours, capstyle="projecting", **kwargs
        )
    ...
    def plotCircularTree(
        ...
        **kwargs,
        ...
        if normaliseHeight == None:
            normaliseHeight = lambda value: (value - min(allXs)) / (
                max(allXs) - min(allXs)
            )
        linspace = (
            lambda start, stop, n: list(
        ...
        for k in filter(target, self.objects):  ## iterate over branches
            ...
            if k.is_node():
                ...
                yl = (
                    circ_s + circ * yl / self.y_span
                )  ## transform y into a fraction of total y
                yr = circ_s + circ * yr / self.y_span
                ybar = linspace(
                    yl, yr, precision
    ...
    def plotCircularPoints(
        ...
        **kwargs,
        ...
        for k in filter(target, self.objects):
            ...
            y = (
                circ_s + circ * y_attr(k) / self.y_span
...
def make_tree(data, ll=None, verbose=False):
    ...
    while i < len(
        ...
        )  ## make sure that you've actually parsed something last time, if not - there's something unexpected in the tree string
        ...
        if cerberus is not None:
            ...
            i += (
                len(cerberus.group(3))
                + cerberus.group().count("'")
                + cerberus.group().count('"')
            )  ## advance in tree string by however many characters the tip is encoded
        ...
        if cerberus is not None:
            if verbose == True:
                print(
                    "%d adding incoming reticulation branch %s" % (i, cerberus.group(1))
                )
            ...
            for k in ll.Objects:  ## iterate over currently existing branches
                if isinstance(k, reticulation) and k.name == cerberus.group(
                    1
...
def make_tree_json(JSONnode, json_translation, ll=None, verbose=False):
